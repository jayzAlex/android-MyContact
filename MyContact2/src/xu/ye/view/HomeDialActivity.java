...
public class HomeDialActivity extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		miss_btn = (Button) findViewById(R.id.miss_btn);
...
//				Log.i(TAG, "HomeDialActivity onItemClick");
				CallLogBean cb = (CallLogBean) adapter.getItem(position);
				ContactBean contactBean = new ContactBean();
				contactBean.setDisplayName(cb.getName());
				contactBean.setPhoneNum(cb.getNumber());
				contactBean.setBlkwhi(getIntent().getStringExtra("blkwhi"));
				if(getIntent().getIntExtra("flag", -1) == HomeContactActivity2.INTENT_SELECT_CONTACTBEAN){
