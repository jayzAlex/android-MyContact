...
public class RexseeSMS {  
	...
	public List<SMSBean> getThreads(int limit, int offset) {  
		Cursor cursor = null;  
		ContentResolver contentResolver = mContext.getContentResolver();  
		List<SMSBean> list=new ArrayList<SMSBean>();
		try {  
			if (limit > 0) {  
				cursor = contentResolver.query(Uri.parse(CONTENT_URI_SMS_CONVERSATIONS), THREAD_COLUMNS, null, null, "date desc limit " + limit + " offset " + offset);  
			} else {  
			...
			if(cursor != null)	
				cursor.close();
//			System.out.println("====getThreads list:" + list.size());
			return list;  
		} catch (Exception e) {  
			return list;  
		}  
	...
	public List<SMSBean> getThreads(int limit) {  
		return getThreads(limit, 0);
	}  
	...
	public List<SMSBean> getThreadsNum(List<SMSBean> ll) {
		...
		for (SMSBean mmt : ll) {
			...
			String displayname = lookupPersonName(mContext, cursor.getString(2));
			mmt.setDisplayName(TextUtils.isEmpty(displayname) ? cursor
					.getString(2) : displayname);
			list.add(mmt);
		}
		if(cursor != null)
			cursor.close();
	...
	public static String lookupPersonName(Context context, String address) {
		String personName = null;
		Uri personUri = Uri.withAppendedPath(
				ContactsContract.PhoneLookup.CONTENT_FILTER_URI, address);
		Cursor phoneCursor = context.getContentResolver().query(personUri,
